...
function ReportDialog({ pkg, open, onOpenChange }: PackageDialogProps) {
  ...
  const handleClick = async () => {
    setIsLoading(true);
    const res = await reportPackageAction({
      name: pkg.name,
      version: pkg.version,
    ...
    if (typeof res?.error === "string") {
      ...
      toast({
        description: `Successfully reported ${pkg.name} v${pkg.version}`,
      });
    }
...
function DetailedInformationDialog({
  pkg,
  open,
  onOpenChange,
}: PackageDialogProps) {
  const buildCommitHashGithubURL = (commitHash: string) =>
    `https://github.com/vipyrsec/security-intelligence/commit/${commitHash}`;
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent onOpenAutoFocus={(e) => e.preventDefault()}>
        ...
        <p>
          Rules: {pkg.rules?.join(", ") || "None"} (Score: {pkg.score})
        </p>
        <p>
          Queued <RelativeTimstamp date={dayjs.unix(pkg.queued_at)} /> by{" "}
          {pkg.queued_by}
        </p>
        {pkg.finished_at && pkg.finished_by ? (
          <p>
            Scanned <RelativeTimstamp date={dayjs.unix(pkg.finished_at)} /> by{" "}
            {pkg.finished_by}
          </p>
        ) : (
          <></>
        ...
        {pkg.commit_hash ? (
          <p>
            ...
            <Link
              href={buildCommitHashGithubURL(pkg.commit_hash)}
              target="_blank"
            >
              {pkg.commit_hash}
            </Link>
